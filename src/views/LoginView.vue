      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
        ...
        <div class="bg-white/90 dark:bg-[#1e293b]/90 backdrop-blur-lg p-8 rounded-l-3xl rtl:rounded-l-none rtl:rounded-r-3xl shadow-2xl">
          <div class="text-center mb-8">
            ...
            <p class="text-base text-gray-600 dark:text-gray-300">{{ $t('auth.loginSubtitle') }}</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="flex items-center justify-between">
              <label class="flex items-center space-x-2 cursor-pointer">
                ...
                />
                <span class="text-sm text-gray-600 dark:text-gray-300">{{ $t('auth.rememberMe') }}</span>
              </label>
              <router-link
                to="/forgot-password"
                class="text-sm text-[#25D366] hover:text-[#128C7E] font-medium"
              >
            ...
            <div class="text-center">
              <p class="text-sm text-gray-600 dark:text-gray-300">
                {{ $t('auth.noAccount') }}
                <router-link to="/register" class="text-[#25D366] hover:text-[#128C7E] font-medium">
                  {{ $t('common.register') }}
                </router-link>
              </p>
            </div>
        ...
        <div class="hidden md:flex items-center justify-center bg-gradient-to-br from-[#25D366] to-[#128C7E] rounded-r-3xl rtl:rounded-r-none rtl:rounded-l-3xl p-8">
          <div class="text-white text-center">
            <div class="mb-8">
              <h2 class="text-4xl font-bold mb-4">{{ $t('auth.welcomeBack') }}</h2>
              <p class="text-lg opacity-90">{{ $t('auth.welcomeMessage') }}</p>
            </div>
...
const validateForm = () => {
  ...
  if (!formData.value.username) {
    errors.value.username = t('errors.required', { field: t('common.email') })
    isValid = false
  } 
